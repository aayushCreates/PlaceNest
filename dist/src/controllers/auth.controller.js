...
const register = async (req, res, next) => {
    try {
        ...
        const token = await (0, auth_middleware_1.getJWT)(newUser.id, newUser.role);
...
const login = async (req, res, next) => {
    try {
        ...
        const isValidUser = await (0, auth_middleware_1.checkValidUserByPassword)(password, user.password);
        if (!isValidUser) {
        ...
        //   });
        // }
        const token = await (0, auth_middleware_1.getJWT)(user.id, user.role);
        res.cookie("token", token, cookieOptions);
        res.status(200).json({
            success: true,
